...
NO_UI_MSG = """The swagger_ui directory could not be found.
    Please install specific with extra install: pip install specific[swagger-ui]
...
class SpecificOptions(object):
    ...
    def extend(self, new_values=None):
        # type: (Optional[dict]) -> SpecificOptions
        """
        Return a new instance of `SpecificOptions` using as default the currently
        defined options.
        """
        ...
        return SpecificOptions(options, self.oas_version)
    ...
    def openapi_console_ui_from_dir(self):
        # type: () -> str
        """
        Custom OpenAPI Console UI directory from where Specific will serve
        the static files.

        Default: Specific's vendored version of the OpenAPI Console UI.
        """
